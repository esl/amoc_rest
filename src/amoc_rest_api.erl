...
validate(Rule = {type, 'integer'}, Name, Value, _ValidatorState) ->
    try
        {ok, amoc_rest_utils:to_int(Value)}
    catch
        error:badarg ->
...
validate(Rule = {type, 'float'}, Name, Value, _ValidatorState) ->
    try
        {ok, amoc_rest_utils:to_float(Value)}
    catch
        error:badarg ->
            validation_error(Rule, Name)
    end;
...
validate(Rule = schema, Name, Value, ValidatorState) ->
    Definition =  list_to_binary("#/components/schemas/" ++ amoc_rest_utils:to_list(Name)),
    try
...
get_value(qs_val, Name, Req) ->
    ...
    Value = amoc_rest_utils:get_opt(amoc_rest_utils:to_qs(Name), QS),
    {Value, Req};

get_value(header, Name, Req) ->
    Headers = cowboy_req:headers(Req),
    Value =  maps:get(amoc_rest_utils:to_header(Name), Headers, undefined),
    {Value, Req};
...
binary_to_lower(V) when is_binary(V) ->
    list_to_binary(string:to_lower(amoc_rest_utils:to_list(V))).
